# Pack dist/pac_maze_sfx and upload to CloudBase COS
# Run build_pac_maze_sfx.ps1 first; requires tcb login

$ErrorActionPreference = "Stop"
[Console]::OutputEncoding = [System.Text.Encoding]::UTF8

$RepoRoot = Split-Path (Split-Path $PSScriptRoot -Parent) -Parent
$SrcDir = Join-Path $RepoRoot "dist\pac_maze_sfx"
$OutDir = Join-Path $RepoRoot "dist\asset-bundles"
$ZipPath = Join-Path $OutDir "pac_maze_sfx.zip"
$CloudPrefix = "yishi-assetss/v1/bundles"

if (-not (Test-Path $SrcDir)) {
    Write-Host "Missing $SrcDir - run build_pac_maze_sfx.ps1 first" -ForegroundColor Red
    exit 1
}

New-Item -ItemType Directory -Force -Path $OutDir | Out-Null

Write-Host "`n=== Zip pac_maze_sfx ===" -ForegroundColor Cyan
if (Test-Path $ZipPath) { Remove-Item $ZipPath -Force }
$stagingParent = Join-Path $RepoRoot "dist\_zip_staging"
$stagingBundle = Join-Path $stagingParent "pac_maze_sfx"
...
Write-Host "-> pac_maze_sfx.zip ($mb MB)"

Write-Host "`n=== Upload to $CloudPrefix ===" -ForegroundColor Cyan
Push-Location (Join-Path $RepoRoot "backend")
$cloudPath = "$CloudPrefix/pac_maze_sfx.zip"
Write-Host "upload pac_maze_sfx.zip -> $cloudPath"
...
Write-Host "COS: yishi-assetss/v1/bundles/pac_maze_sfx.zip"
Write-Host "Redeploy asset_bundle cloud function, then App can fetch bundleId=pac_maze_sfx"
